...
class PhylogicOutput(object):
                ...
                if treatment_file:
                    ...
                    with open(treatment_file, 'r') as f:
                        ...
                        for line in f:
                            ...
                            treatment_data.append({'tx': fields['tx'],
                                                   'tx_start': float(fields['tx_start']),
                                                   'tx_end': float(fields['tx_end']) if fields['tx_end'] else
                                                   timepoints[-1]})
                if tumor_sizes_file:
    ...
    def write_html_report(cls, patient, sample_names, cluster_dict, aliases=None, time_points=None, pie_plots=None,
        ...
        if time_points:
            ...
            time_switch_options['show'] = time_points != list(range(len(sample_names))) and len(
                np.unique(time_points)) > 1
        ...
        if treatment_data:
            treatment_plot_options = {'treatments': treatment_data, 'samples': time_points,
                                      'treatment_color_map': {s: ClusterColors.get_rgb_string(i + len(cluster_dict)) for
                                                              i, s in
                                                              enumerate(np.unique([x['tx'] for x in treatment_data]))}}
        ...
        with open('{}.cluster_ccfs.txt'.format(patient.indiv_name), 'w') as f:
            ...
            for cluster in cluster_ccfs:
                for i, sample in enumerate(patient.sample_list):
                    ...
                    line = [patient.indiv_name, sample.sample_name, aliases[i], str(cluster), str(mean), str(low),
                            str(high)]
            ...
            for sample_id, cluster_densitites in mcmc_trace_constrained_densitites.items():
                for cluster_id, densities in cluster_densitites.items():
                    for iteration, constrained_density in enumerate(densities):
                        line = [indiv_id, str(sample_id), str(iteration), str(cluster_id)] + [str(x) for x in
                                                                                              constrained_density]
                        writer.write('\t'.join(line) + '\n')
